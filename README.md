...
Docker container for building personal sites or chats.
...
-e PG_HOST=db \
-e PG_PORT=5432 \
-e PG_DATABASE=site_db \
-e PG_USER=user \
-e PG_PASSWORD=password \
-d perfumerlabs/site:v1.1.0
```
...
This microservice is invented to provide data structure and API for personal sites like notification site,
some kind of article site with personal algorithms, chats and so on.

When a record appears you push it to Site. Then you can fetch a number of records
from the top of list or fetch a particular record and send it to client-side.
Site has integration with websocket service [Centrifugo](https://github.com/centrifugal/centrifugo) and [Badges](https://github.com/perfumerlabs/badges) microservice.
If config for those integrations is set, then Site will send pushes to the service by itself.
Also Site will automatically delete badges from [Badges](https://github.com/perfumerlabs/badges) when read API is called.
