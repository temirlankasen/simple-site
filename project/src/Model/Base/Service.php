...
abstract class Service implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Site\\Model\\Map\\ServiceTableMap';
    ...
    /**
     * Initializes internal state of Site\Model\Base\Service object.
     */
     ...
     * @return $this|\Site\Model\Service The current object (for fluent API support)
     */
    ...
    public function setDigest($v)
    {
        ...
        if ($this->digest !== $v) {
            $this->digest = $v;
            $this->modifiedColumns[ServiceTableMap::COL_DIGEST] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ServiceTableMap::translateFieldName('Digest', TableMap::TYPE_PHPNAME, $indexType)];
            $this->digest = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ServiceTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = ServiceTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Site\\Model\\Service'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('service_id_seq')");
        ...
        if ($this->isColumnModified(ServiceTableMap::COL_DIGEST)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Site\Model\Service
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Site\Model\Service The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ServiceTableMap::COL_DIGEST)) {
            $criteria->add(ServiceTableMap::COL_DIGEST, $this->digest);
        }
        if ($this->isColumnModified(ServiceTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Site\Model\Service (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Site\Model\Service Clone of current object.
     * @throws PropelException
     */
