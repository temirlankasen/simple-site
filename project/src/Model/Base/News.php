...
abstract class News implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Site\\Model\\Map\\NewsTableMap';
    ...
    /**
     * Initializes internal state of Site\Model\Base\News object.
     */
     ...
     * @return $this|\Site\Model\News The current object (for fluent API support)
     */
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[NewsTableMap::COL_NAME] = true;
        }
    ...
    public function setDigest($v)
    {
        ...
        if ($this->digest !== $v) {
            $this->digest = $v;
            $this->modifiedColumns[NewsTableMap::COL_DIGEST] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : NewsTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : NewsTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            $this->description = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : NewsTableMap::translateFieldName('Digest', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = NewsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Site\\Model\\News'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('news_id_seq')");
        ...
        if ($this->isColumnModified(NewsTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(NewsTableMap::COL_DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = 'description';
        }
        if ($this->isColumnModified(NewsTableMap::COL_DIGEST)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Site\Model\News The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(NewsTableMap::COL_NAME)) {
            $criteria->add(NewsTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(NewsTableMap::COL_DESCRIPTION)) {
            $criteria->add(NewsTableMap::COL_DESCRIPTION, $this->description);
        }
        if ($this->isColumnModified(NewsTableMap::COL_DIGEST)) {
            $criteria->add(NewsTableMap::COL_DIGEST, $this->digest);
        }
        if ($this->isColumnModified(NewsTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Site\Model\News (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Site\Model\News Clone of current object.
     * @throws PropelException
     */
